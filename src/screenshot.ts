...
function createEmptyCanvas(scale: number) {
...
export type SuccessFn = (url: string) => void;

export default function (img: HTMLImageElement, scale: number, success: SuccessFn) {
  const cvs = createEmptyCanvas(scale);
  const ctx = cvs.getContext('2d')!;
  const cvsUtil = new CvsUtil(cvs);
  const toolBar = new ToolBar(cvs);
  let areaStart: Point | null;
  ...
  cvs.onmousemove = (e: MouseEvent) => (mouse = Point.fromMouse(e, scale));
